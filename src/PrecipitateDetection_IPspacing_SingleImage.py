...
============================================================
 ...
 Condition : 725 C, 1 hr
============================================================
 Workflow
 --------
 1. Load micrograph
 2. Detect + validate small bright precipitates (top-hat + LoG blobs)
 3. Compute statistical descriptors:
       - Equivalent diameter, d        (mean, std dev, median, min, max)
       - Nearest-neighbour spacing, s  (mean, std dev, median, min, max)
 4. Print descriptors using plain statistical notation
 5. Show ONE summary figure (detection overlay, diameter histogram,
    spacing histogram, nearest-neighbour map, spacing heat-map)

 NOTE: No volume-percent plot and no area-fraction plot/metric are
       generated at all in this script.
============================================================
"""
...
EDGE_TO_EDGE      = False   # True -> edge-to-edge NN spacing, False -> centre-to-centre
...
# ============================================================
# STEP 1 -- Pre-processing for tiny bright precipitates
# ============================================================

def preprocess(img_gray, tophat_radius=2, smooth_sigma=0.35):
...
# ============================================================
# STEP 2 -- Blob detection tuned for small particles only
# ============================================================

def detect_blobs(
    tophat_smooth,
    min_sigma=0.55, max_sigma=2.50, num_sigma=25,
    threshold=0.018, overlap=0.55,
    min_diameter_px=2.0, max_diameter_px=9.0,
):
    blobs = blob_log(
        tophat_smooth,
...
# ============================================================
# STEP 3 -- Validate small, bright, round precipitates only
# ============================================================

def validate_precipitates(
    img_gray, tophat_norm, candidates_df,
    local_radius_factor=2.8, threshold_fraction=0.45,
    min_peak_brightness=0.45, min_contrast_ratio=2.0,
    min_area_local=2, max_area_local=65,
    min_equiv_diameter=1.5, max_equiv_diameter=9.5,
    max_aspect_ratio=1.65, min_circularity=0.45,
    min_solidity=0.65, max_eccentricity=0.72,
    min_fill_fraction=0.03, max_component_count=3,
    mask_radius_factor=1.20,
):
    accepted = []
    ...
    for _, row in candidates_df.iterrows():
        ...
        r_blob = max(1, int(round(row["radius_px"])))
        r = max(4, int(round(row["radius_px"] * local_radius_factor)))
        ...
        annulus_mask = (dist_sq > (r_blob * 1.6) ** 2) & (dist_sq <= r ** 2)
        ...
        contrast_ratio = np.inf if annulus_mean < 1e-6 else core_vals.mean() / annulus_mean
        ...
        equiv_diam = sel.equivalent_diameter_area
        ...
        if not (
            ...
            and min_equiv_diameter <= equiv_diam <= max_equiv_diameter
        ...
        accepted.append({
            ...
            "area_local": area, "equivalent_diameter_px": equiv_diam,
    ...
    print(f"After small bright round-particle filter : {len(vdf)}")
...
# ============================================================
# STEP 4 -- Statistical descriptors
# ============================================================
...
# ============================================================
# STEP 5 -- Summary plot (5 panels, centered layout, no overlap)
# ============================================================

def plot_summary(img_gray, validated_df, result_df, diam_stats, spacing_stats):
    ...
    fig.suptitle(
        f"Precipitate Analysis  |  Condition: 725 C, 1 hr  |  Sample size n = {diam_stats['n']}",
...
# ============================================================
# STEP 6 -- Pretty printer (proper statistical notation)
# ============================================================

def print_summary(diam_stats, spacing_stats, mpp, npp):
    ...
    print("")
    print(line)
    print("  PRECIPITATE ANALYSIS SUMMARY  --  725 C, 1 hr")
...
# ============================================================
# MAIN -- load, run, print, plot. That's it.
# ============================================================
...
if __name__ == "__main__":
    ...
    if len(validated_df) == 0:
        raise ValueError("No precipitates were validated -- check detection parameters / image path.")
    ...
    print_summary(d_stats, s_stats, mpp, npp)
    plot_summary(img_gray, validated_df, result_df, d_stats, s_stats)
